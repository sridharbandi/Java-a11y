...
public interface IRunner {
    ...
    default void generateHtmlReport(A11y a11y, Engine engine, Class<?> clazz) throws IOException {
        ...
        issuesList.forEach(issues -> {
            ...
            String name = engine.name().equalsIgnoreCase("axe") ? ((Issues) issues).getTitle() : ((io.github.sridharbandi.modal.htmlcs.Issues) issues).getTitle();
            a11y.save(tmplPage, issues, name, engine);
        });
